...
export function buildDirectSynthesisPrompt(
  ...
  const identityList = userIdentities.length > 0
    ...
    : userWxid;
  return `你是情报分析员，以美军 PDB（总统每日简报）标准为决策者（你的领导）撰写**${date}** 的日报。
...
8. **第三方 vs 领导本人 — 严格区分**（极易出错的雷区）：
   - 群里别人**宣布自己家事**（"我家孩子录取了 X 大"、"我搬家了"、"我老婆生了"）—— **绝对不是领导的家事**，要写"**X 群成员 Y 的孩子**录取..." 或 "**Y 分享：他家**搬家"，**禁止**把第三方家事算到领导头上
   - 群里别人**宣布工作事**（"我们公司融资了"、"我升职了"）—— **绝对不是领导的工作事**
   - 只有当消息发送人 wxid 等于"领导身份列表"中的某个 wxid，或者消息内容里**直接 @ 领导某个名字**，才能视为领导本人的事
   - 如果不确定，宁可写"**未明确：可能是 X 的家事**"也不要假设是领导的事
   - 这条规则**优先级最高**，违反就是事实性错误，比聚类错误严重十倍

【输出结构 — 注意：不要输出 H1 标题（# 微信日报 …），从 H2 开始即可，简报本身的 H1 由系统添加】
