...
- **禁止全部打 N 的懒惰行为**：每条证据**至少要对一个假设给出 C 或 I**；若证据完全无判别力（与所有假设都无关），你应该质疑这条证据是否该入矩阵 —— 但仍请给至少一个 C 或 I（选最接近的那个假设）
- **合理打 C**：证据和假设实体/机制直接相关时即可打 C，别过度保守
- 矩阵里 C+I 标签总数应至少占 40%；若你最终产出的矩阵 C+I 占比 <30%，说明评估过于保守，请重来
